...
(function (window) {
  ...
  ══════════════════════════════════════════════════════════════ */
  function parseGroupWaypoints(groupContent, startTime, task, theatre, acType) {
    const secsAbsToHHMM = window.secsAbsToHHMM;
    const wps = [];
    let toTime = '', toTime_abs = '', totTime = '', airdrome = null, landingAirdrome = null;

    /* Numérotation WP : certains avions (F-14) commencent à WP 0 */
    let wpIdx = WP_ZERO_BASED.has(acType) ? 0 : 1;

    const rcM = groupContent.match(/\["route"\]\s*=\s*\{([\s\S]*?)\n\s*\},\s*--\s*end of \["route"\]/);
    if (!rcM) return { wps, toTime, toTime_abs, totTime, airdrome, landingAirdrome };
    let rc = rcM[1];
  ...
  /* ══════════════════════════════════════════════════════════════
     AVIONS DONT LA NUMÉROTATION DES WAYPOINTS COMMENCE À 0
     (WP[1] Lua = WP 0 dans le kneeboard)
  ══════════════════════════════════════════════════════════════ */
  const WP_ZERO_BASED = new Set(['F-14A', 'F-14A-135-GR', 'F-14B']);
  ...
  window.parseMiz = function (content, theatre, dictionary = {}) {
    ...
    for (const { key: coalKey, name: coalName } of COAL_MAP) {
      ...
      while ((gm = groupIdRe.exec(coalBlock)) !== null) {
        ...
        /* ── Waypoints ── */
        const { wps, toTime, toTime_abs: entry_toTimeAbs, totTime, airdrome: rawAirdrome, landingAirdrome: rawLandingAirdrome } = parseGroupWaypoints(gc, res.start_time, task, theatre, acType);
